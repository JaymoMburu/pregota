...
.qr-card p{font-size:12px;color:rgba(255,255,255,.78);margin-bottom:20px}
...
th{padding:9px 14px;text-align:left;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:rgba(255,255,255,.78);background:rgba(255,255,255,.03)}
...
.empty{text-align:center;padding:36px;color:rgba(255,255,255,.72);font-size:13px}
...
.stamp-section p{font-size:12px;color:rgba(255,255,255,.78);margin-bottom:16px;line-height:1.55}
.form-row{display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap}
.form-group-sm{display:flex;flex-direction:column;gap:5px}
.form-group-sm label{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:rgba(255,255,255,.78)}
...
<nav class="nav">
    ...
    <div class="nav-right">
        ...
        <form method="POST" action="{{ route('seller.logout') }}" style="display:inline">
            @csrf
            <button type="submit" style="background:none;border:none;color:rgba(255,255,255,.78);font-size:13px;cursor:pointer">Logout</button>
        </form>
    </div>
</nav>
...
<div class="main">
    ...
    <div class="stamp-section" style="background:rgba(251,191,36,.04);border-color:rgba(251,191,36,.2)">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px" id="fare-list">
            ...
            @endforeach
            @if($fares->isEmpty())
            <div id="no-fares-msg" style="font-size:12px;color:rgba(255,255,255,.65)">No fare stages yet. Add your first stop below.</div>
            @else
            <div id="no-fares-msg" style="display:none;font-size:12px;color:rgba(255,255,255,.65)">No fare stages yet. Add your first stop below.</div>
            @endif
        </div>
    ...
    <div class="stamp-section">
        ...
        <form method="POST" action="{{ route('seller.stamp-card') }}">
            ...
            @else
            <div style="margin-top:10px;font-size:11px;color:rgba(255,255,255,.65)">Stamp card is off. Set a number above to activate it.</div>
            @endif
        </form>
    </div>
    ...
    <div class="stamp-section" style="background:rgba(239,68,68,.04);border-color:rgba(239,68,68,.15)">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:14px;margin-bottom:2px">
            <div style="background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.15);border-radius:10px;padding:12px;text-align:center">
                ...
                <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:rgba(255,255,255,.65);margin-top:3px">Outstanding</div>
            </div>
            <div style="background:rgba(37,211,102,.05);border:1px solid rgba(37,211,102,.12);border-radius:10px;padding:12px;text-align:center">
                ...
                <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:rgba(255,255,255,.65);margin-top:3px">Collected</div>
            </div>
            <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:12px;text-align:center">
                ...
                <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:rgba(255,255,255,.65);margin-top:3px">Open Tabs</div>
            </div>
        </div>
        @endif
            ...
            <div style="background:rgba(239,68,68,.04);border:1px solid rgba(239,68,68,.12);border-radius:10px;overflow:hidden">
                <div style="padding:12px 14px">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:6px">
                        <div>
                            ...
                            <div style="font-size:11px;color:rgba(255,255,255,.72)">
                        ...
                        <div style="display:flex;gap:6px;flex-wrap:wrap">
                            ...
                            <button onclick="navigator.clipboard.writeText('{{ url('/deni/' . $d->debtor_token) }}');this.textContent='✓ Copied!'" style="font-size:11px;padding:5px 10px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:6px;color:rgba(255,255,255,.78);cursor:pointer">Copy Link</button>
                            <a href="{{ url('/deni/admin/' . $d->admin_token) }}" style="font-size:11px;padding:5px 10px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:6px;color:rgba(255,255,255,.78);text-decoration:none">View →</a>
                        </div>
                    </div>
                ...
                <div id="charge-{{ $d->admin_token }}" style="display:none;border-top:1px solid rgba(239,68,68,.15);padding:12px 14px;background:rgba(239,68,68,.06)">
                    <form method="POST" action="{{ route('deni.charge', $d->admin_token) }}" style="display:flex;gap:8px;flex-wrap:wrap;align-items:flex-end">
                        ...
                        <button type="button" onclick="toggleCharge('{{ $d->admin_token }}')" style="padding:8px 12px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:7px;color:rgba(255,255,255,.72);font-size:12px;cursor:pointer">Cancel</button>
                    </form>
                </div>
            </div>
            @endforeach
        ...
        <div style="margin-top:16px;display:flex;flex-direction:column;gap:8px">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:rgba(168,85,247,.06);border:1px solid rgba(168,85,247,.15);border-radius:10px;flex-wrap:wrap;gap:8px">
                <div>
                    ...
                    <div style="font-size:12px;color:rgba(255,255,255,.72)">KES {{ number_format($plan->amount) }} / {{ $plan->frequencyLabel() }} · {{ $plan->subscriptions_count }} subscriber{{ $plan->subscriptions_count == 1 ? '' : 's' }}</div>
                </div>
    ...
    <div class="table-wrap">
        <div class="table-header">
            ...
            <span style="color:rgba(255,255,255,.78);font-size:12px">Last 50</span>
        </div>
        @if($payments->isEmpty())
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="color:rgba(255,255,255,.78)">KES {{ number_format($p->fee) }}</td>
                    <td style="color:#4ade80;font-weight:700">KES {{ number_format($p->net_amount) }}</td>
                    <td style="color:rgba(255,255,255,.78);font-size:12px">{{ $p->buyer_note ?: '—' }}</td>
                    ...
                    <td style="font-size:12px;color:rgba(255,255,255,.78)">{{ $p->created_at->format('d M, H:i') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
