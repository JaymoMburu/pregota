...
<title>{{ $plan->name }} Subscribers — Pregota</title>
...
.badge.cancelled{background:rgba(255,255,255,.06);color:rgba(255,255,255,.72)}
...
.date{font-size:11px;color:rgba(255,255,255,.65);margin-top:2px}
...
<div class="wrap">
    ...
    <div class="share-box">
        ...
        <button class="copy-btn" onclick="navigator.clipboard.writeText('{{ url('/subscribe/' . $plan->id) }}');alert('Copied!')">📋 Copy</button>
    </div>
    ...
    @if($subscribers->isEmpty())
        <div style="text-align:center;padding:40px;color:rgba(255,255,255,.65)">No subscribers yet. Share the subscribe link to get started.</div>
    @else
        ...
        <div class="row {{ $s->status }}">
            <div>
                <div style="display:flex;align-items:center;gap:8px">
                    ...
                    @if($s->next_due_at)<span style="font-size:11px;color:rgba(255,255,255,.72)">Next due: {{ $s->next_due_at->format('d M Y') }}</span>@endif
                </div>
