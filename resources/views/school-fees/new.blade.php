...
<title>Set Up School Fees Collection — Pregota</title>
<meta name="csrf-token" content="{{ csrf_token() }}">
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Segoe UI',system-ui,sans-serif;background:#0f0f1a;color:#fff;min-height:100vh;display:flex}
.panel-left{width:42%;min-height:100vh;background:linear-gradient(150deg,#0a0015,#1e0840 55%,#2d0a4e);position:relative;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between;padding:40px 44px}
.blob{position:absolute;border-radius:50%;filter:blur(100px);pointer-events:none}
.b1{width:420px;height:420px;background:#7c3aed;opacity:.22;top:-120px;left:-80px}
.b2{width:300px;height:300px;background:#db2777;opacity:.18;bottom:0;right:-40px}
...
.flow-num{width:26px;height:26px;border-radius:50%;background:linear-gradient(135deg,#7c3aed,#db2777);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:900;flex-shrink:0;margin-top:1px}
...
.class-row input:focus{border-bottom-color:#7c3aed;background:transparent}
...
.add-class-btn:hover{border-color:#7c3aed;color:#c084fc;background:rgba(124,58,237,.06)}
...
.submit-btn{width:100%;padding:15px;border-radius:12px;border:none;font-size:16px;font-weight:700;cursor:pointer;background:linear-gradient(135deg,#7c3aed,#db2777);color:#fff;margin-top:10px;transition:.2s}
...
<div class="panel-left">
    ...
    <div class="left-center">
        <div class="headline">
            <h1>School fees.<br><em>No cash handling.<br>No reconciliation.</em></h1>
            <p>Parents pay directly to the school via M-Pesa. Class teachers share a link. Admin sees everything in real time.</p>
        </div>
        <div class="flow-steps">
            ...
            <div class="flow-step"><div class="flow-num">4</div><div class="flow-text"><strong>Admin pays out when ready</strong><span>One click sends the total directly to the school M-Pesa. Every Thursday — done in seconds.</span></div></div>
        </div>
    </div>
            ...
            <form method="POST" action="{{ route('school-fees.store') }}">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="text" name="term_label" placeholder="Term 2 · 2026"
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="hint green">🔒 Encrypted. Deleted after payout.</div>
                    </div>
                </div>
                ...
                <div class="classes-builder" id="classesBuilder">
                    <div class="class-row">
                        ...
                        <button type="button" class="remove-class" onclick="removeClass(this)" title="Remove">×</button>
                    </div>
                </div>
                <button type="button" class="add-class-btn" onclick="addClass()">+ Add Another Class</button>

                <button type="submit" class="submit-btn">Create Collection & Get Links →</button>
                <div style="text-align:center;margin-top:10px;font-size:11px;color:rgba(255,255,255,.25)">KES 30 fee per payment · Added on top of the amount · Paid by parent</div>
            </form>
        </div>
    </div>
</div>
