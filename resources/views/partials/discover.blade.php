...
$modules = [
    ...
    'collection' => ['emoji' => '💬', 'name' => 'WhatsApp Collections', 'desc' => 'Post a link, group pays directly', 'url' => route('collection.landing')],
...
<div style="background:rgba(255,255,255,.02);border-top:1px solid rgba(255,255,255,.07);padding:40px 24px">
    <div style="max-width:680px;margin:0 auto">
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:rgba(255,255,255,.82);margin-bottom:16px;text-align:center">Also on Pregota</div>
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px">
            @foreach($_others as $mod)
            <a href="{{ $mod['url'] }}" style="display:flex;flex-direction:column;align-items:center;gap:6px;padding:16px 10px;border-radius:12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);text-decoration:none;transition:.15s" onmouseover="this.style.borderColor='rgba(0,166,81,.4)';this.style.background='rgba(0,166,81,.1)'" onmouseout="this.style.borderColor='rgba(255,255,255,.08)';this.style.background='rgba(255,255,255,.04)'">
                ...
                <span style="font-size:11px;color:rgba(255,255,255,.6);text-align:center;line-height:1.4">{{ $mod['desc'] }}</span>
            </a>
            @endforeach
        </div>
    </div>
...
<div style="margin-top:20px;padding-top:16px;border-top:1px solid rgba(255,255,255,.08);position:relative;z-index:1">
    ...
    <div style="display:flex;flex-direction:column;gap:6px">
        @foreach($_others as $mod)
        <a href="{{ $mod['url'] }}" style="display:flex;align-items:center;gap:10px;padding:9px 12px;border-radius:10px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);text-decoration:none;transition:.15s" onmouseover="this.style.borderColor='rgba(0,166,81,.3)';this.style.background='rgba(0,166,81,.1)'" onmouseout="this.style.borderColor='rgba(255,255,255,.07)';this.style.background='rgba(255,255,255,.04)'">
            ...
            <div style="flex:1;min-width:0">
                ...
                <span style="font-size:11px;color:rgba(255,255,255,.6)">{{ $mod['desc'] }}</span>
            </div>
